...
## GRPC接口注册位置
service/serve/registerGRPCService.go
## GinRouter接口注册位置
service/serve/registerGinRouter.go
...
# 编译后端
go build -o moredoc main.go（这个意思是编译一个名字叫做moredoc的exe文件）
go build main.go(自己设置的，不用运行)
# 初始化数据库结构
./moredoc syncdb

# 运行后端(可用其他热编译工具)，监听8880端口
go run main.go serve
./moredoc.exe serve(自己设置的，要运行这个才能成功)
./main.exe serve(自己设置的，要运行这个才能成功)
```
